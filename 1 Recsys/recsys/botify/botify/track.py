...
class Catalog:
    ...
    def load(self, catalog_path, top_tracks_path, tracks_with_diverse_recs_path, linear_path):
        ...
        self.app.logger.info(f"Loading tracks with linear recs from {linear_path}")
        # your code is here
        with open(linear_path) as conv_file:
            for j, line in enumerate(conv_file):
                data = json.loads(line)
                self.linear_tracks.append(
                    Track(data["track"], data["artist"], data["title"], data["recommendations"], ))
        self.app.logger.info(f"Loaded {j + 1} tracks with linear recs")
    ...
    def upload_tracks(self, redis_tracks, redis_tracks_with_diverse_recs, redis_linear_tracks):
        ...
        for track in self.linear_tracks:
            redis_linear_tracks.set(track.track, self.to_bytes(track))

        self.app.logger.info(
            f"Uploaded {len(self.tracks)} tracks, {len(self.linear_tracks)} tracks with linear recs"
        )
